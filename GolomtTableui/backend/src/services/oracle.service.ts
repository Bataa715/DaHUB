...
export class OracleService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    // Non-blocking: create pool in background so NestJS startup is not delayed
  ...
  async query<T = Record<string, any>>(sql: string, params: any[] = []): Promise<T[]> {
    ...
    for (const kw of dangerous) {
      if (new RegExp(`\\b${kw}\\b`, 'i').test(trimmed)) {
        this.logger.error(`BLOCKED dangerous keyword "${kw}" in query: ${trimmed.substring(0, 120)}`);
