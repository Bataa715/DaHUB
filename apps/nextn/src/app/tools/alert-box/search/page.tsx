...
function SearchContent() {
  ...
  const doSearch = async (cifVal: string, from?: string, to?: string) => {
    ...
    setExpanded({});
    try {
      const data = await abSearchByCif(cifVal.trim(), from || undefined, to || undefined);
      setResult(data);
    } catch (e: any) {
    ...
    <div className="space-y-5">
      ...
      {searching && (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="text-[12px] text-txt-dim ml-3">12 Dashboard хайж байна...</span>
        </div>
      )}
...
export default function SearchPage() {
  return (
    <Suspense fallback={<div className="p-6 text-txt-dim text-sm">Ачааллаж байна...</div>}>
      <SearchContent />
    </Suspense>
  );
}
