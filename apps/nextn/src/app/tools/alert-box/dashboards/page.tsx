...
function CustomTooltip({ active, payload }: { active?: boolean; payload?: any[]; label?: string; hasAmount?: boolean }) {
  ...
  return (
    <div className="bg-card border border-border rounded-lg px-3 py-2 text-[11px] shadow-xl">
      ...
      <p className="text-muted-foreground">
        Гүйлгээ: <span className="text-foreground font-semibold">{fmtFull(d.count)}</span>
      </p>
      {isAmt && d.totalAmount > 0 && (
        <p className="text-muted-foreground">
          Дүн: <span className="text-amber-400 font-semibold">₮{fmtFull(d.totalAmount)}</span>
        </p>
      )}
    </div>
  );
...
function StatCard({ label, value, sub, icon, loading }: {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 flex items-start gap-3">
      <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 bg-muted/40">
        {icon}
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-wide">{label}</p>
        {loading
          ? <div className="h-5 w-20 rounded bg-muted/40 animate-pulse mt-1" />
...
function DetailPanel({ d, detail, search, onSearch, onRetry }: {
  d: Dashboard; detail: DetailState;
  search: string; onSearch: (v: string) => void; onRetry: () => void;
}) {
  return (
    <div className="px-4 pb-4 pt-3 space-y-3 border-t border-border/50 bg-muted/10">
      <div className="relative max-w-xs">
        <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
        <input value={search} onChange={(e) => onSearch(e.target.value)} placeholder="CIF хайх…"
      ...
      {detail.status === "error" && (
        <div className="rounded-lg border border-red-500/25 bg-red-500/5 px-3 py-3 space-y-1.5">
          ...
          <p className="text-[10px] text-red-300/70 font-mono whitespace-pre-wrap break-all">{detail.error}</p>
      ...
      {detail.status === "done" && detail.rows.length === 0 && (
        <p className="text-[11px] text-muted-foreground text-center py-8">Өгөгдөл олдсонгүй</p>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-card p-3">
            <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide mb-3">
              {detail.hasAmount ? "Гүйлгээний дүн (₮) — Top CIFs" : "Гүйлгээний тоо — Top CIFs"}
            </p>
            <ResponsiveContainer width="100%" height={Math.max(180, detail.rows.length * 30)}>
              <BarChart data={[...detail.rows].reverse()} layout="vertical"
                margin={{ top: 0, right: 16, left: 4, bottom: 0 }}>
                <XAxis type="number" tick={{ fontSize: 9, fill: "hsl(var(--muted-foreground))" }}
                  tickFormatter={fmtShort} axisLine={false} tickLine={false} />
                <YAxis type="category" dataKey="cif" width={82}
                  tick={{ fontSize: 9, fontFamily: "monospace", fill: "hsl(var(--muted-foreground))" }}
                  axisLine={false} tickLine={false} />
                <Tooltip content={<CustomTooltip />} cursor={{ fill: "rgba(255,255,255,0.03)" }} />
                <Bar dataKey={detail.hasAmount ? "totalAmount" : "count"} radius={[0, 5, 5, 0]}>
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="bg-muted/30 border-b border-border">
                  <th className="px-3 py-2 text-left text-muted-foreground font-medium w-7">#</th>
                  <th className="px-3 py-2 text-left text-muted-foreground font-medium">CIF</th>
                  <th className="px-3 py-2 text-right text-muted-foreground font-medium">
                    <span className="flex items-center justify-end gap-1"><Hash size={9} />Тоо</span>
                  </th>
                  {detail.hasAmount && (
                    <th className="px-3 py-2 text-right text-muted-foreground font-medium">
                      <span className="flex items-center justify-end gap-1"><TrendingUp size={9} />Дүн (₮)</span>
                    </th>
                  )}
              ...
              <tbody>
                {detail.rows.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-border/40 hover:bg-muted/20 transition-colors last:border-0">
                      <td className="px-3 py-2 text-muted-foreground/50 tabular-nums">{i + 1}</td>
                      <td className="px-3 py-2 font-mono text-violet-400 font-medium">{row.cif}</td>
                      <td className="px-3 py-2 text-right font-mono tabular-nums">{fmtFull(row.count)}</td>
                      {detail.hasAmount && (
          ...
          <p className="text-[10px] text-muted-foreground/40 text-right font-mono">{d.tableName}</p>
        </div>
      )}
    </div>
  );
...
function DashboardCard({ d, summary, isOpen, detail, search, onToggle, onSearch, onRetry }: {
  d: Dashboard; summary?: Summary; isOpen: boolean;
  detail: DetailState; search: string;
  onToggle: () => void; onSearch: (v: string) => void; onRetry: () => void;
}) {
  const isML = ML_IDS.has(d.id);
  return (
    <div className={`rounded-xl border transition-all overflow-hidden ${
      isOpen ? "border-violet-500/40 shadow-[0_0_0_1px_rgba(139,92,246,0.08)]" : "border-border"}`}>
      <button onClick={onToggle}
        className="w-full flex items-center gap-3 px-4 py-3.5 text-left bg-card hover:bg-muted/20 transition-colors">
        <span className={`flex-shrink-0 inline-flex items-center justify-center gap-0.5 w-12 h-6 rounded-md font-bold text-[10px] border ${
          isML ? "bg-amber-500/10 border-amber-500/25 text-amber-400" : "bg-violet-500/10 border-violet-500/20 text-violet-400"}`}>
          {isML && <Brain size={9} />}
          {isML ? `ML${d.id}` : `DB${d.id}`}
        </span>
        <span className="flex-1 text-[12px] font-medium leading-snug">{d.name}</span>
        <div className="flex items-center gap-3 flex-shrink-0">
          {summary?.totalCount != null ? (
            <span className="text-[11px] font-mono text-muted-foreground tabular-nums flex items-center gap-0.5">
              <Hash size={9} />{fmtShort(summary.totalCount)}
            </span>
          ) : summary?.error ? (
      ...
      {isOpen && (
        <DetailPanel d={d} detail={detail} search={search}
...
export default function DashboardsPage() {
  ...
  const loadDetail = useCallback((id: number, s = "") => {
    ...
    abFetchDashboardTop(id, 10, s)
      ...
      .catch((e) =>
        setDetail({ status: "error", hasAmount: false, rows: [], error: e?.message || "Oracle алдаа" }),
      );
  }, []);

  const handleToggle = (id: number) => {
    if (openId === id) { setOpenId(null); setDetailSearch(""); return; }
    setOpenId(id);
    setDetailSearch("");
    loadDetail(id, "");
  };
  ...
  const nonMlSummaries = summaries.filter((s) => !ML_IDS.has(s.id));
  const mlSummaries = summaries.filter((s) => ML_IDS.has(s.id));
  const totalCount = nonMlSummaries.reduce((a, s) => a + (s.totalCount ?? 0), 0);
  const totalAmount = nonMlSummaries.filter((s) => s.hasAmount).reduce((a, s) => a + (s.totalAmount ?? 0), 0);
  const mlCount = mlSummaries.reduce((a, s) => a + (s.totalCount ?? 0), 0);
  ...
  const filteredStd = listSearch
    ? standardDash.filter((d) => d.name.toLowerCase().includes(listSearch.toLowerCase()) || String(d.id).includes(listSearch))
    : standardDash;
  const filteredML = listSearch
    ? mlDash.filter((d) => d.name.toLowerCase().includes(listSearch.toLowerCase()) || String(d.id).includes(listSearch))
    : mlDash;
            ...
            {filteredStd.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 px-1">
                  ...
                  <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Стандарт загвар</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-violet-500/10 text-violet-400 border border-violet-500/20 font-mono">{filteredStd.length}</span>
                </div>
                <div className="space-y-2">
                  {filteredStd.map((d) => (
                    <DashboardCard key={d.id} d={d} summary={summaryMap[d.id]}
                      isOpen={openId === d.id}
                      detail={openId === d.id ? detail : { status: "idle", hasAmount: false, rows: [] }}
            ...
            {filteredML.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 px-1">
                  ...
                  <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">ML загварт суурилсан</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 font-mono">{filteredML.length}</span>
                  <span className="text-[10px] text-muted-foreground/50 ml-1">· нийт дүнгийн тооцоолоос хасагдсан</span>
                </div>
                <div className="flex items-start gap-2 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-2.5">
                  ...
                  <p className="text-[10.5px] text-amber-300/80 leading-relaxed">
                    Эдгээр dashboard-ууд нь ID 1–12-ын стандарт загваруудтай ижил өгөгдлийг ML загвараар
                    дахин боловсруулсан тул нийлбэр тооцоолол давхардахаас зайлсхийж нийт дүнд оруулаагүй.
                  </p>
                </div>
                <div className="space-y-2">
                  {filteredML.map((d) => (
