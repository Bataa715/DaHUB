...
import { X, TrendingUp, Loader2, GitCompare, Table, BarChart2, Hash, Database } from "lucide-react";

const PALETTE = [
  "#3b82f6","#10b981","#8b5cf6","#f59e0b","#ec4899","#06b6d4",
  "#ef4444","#84cc16","#f97316","#6366f1","#14b8a6","#e11d48",
  "#7c3aed","#0ea5e9","#d97706","#16a34a","#dc2626","#9333ea",
  "#0891b2","#ca8a04",
];
...
export default function ComparePanel({
  ...
  const [loadedMap, setLoadedMap] = useState<Map<string, RiskCurrentRow[]>>(new Map());
  ...
  useEffect(() => {
    ...
    if (!toFetch.length) { setLoadedMap(new Map(cacheRef.current)); return; }
    setFetching(true);
    Promise.all(
      toFetch.map((id) =>
        riskApi.getHistory(id)
          .then((res) => { cacheRef.current.set(id, res.rows ?? []); })
          .catch(() => { cacheRef.current.set(id, []); }),
      ),
    ).finally(() => { setLoadedMap(new Map(cacheRef.current)); setFetching(false); });
  }, [open, selectedIds]);
  ...
  const allBranches = useMemo(() => {
    ...
    for (const h of selectedReports) {
      ...
      if (metricMode === "total") {
        ...
        for (const r of rows) {
          if (r.BRANCHID && !m.has(r.BRANCHID))
            m.set(r.BRANCHID, String(r.BRANCHNAME ?? r.BRANCHID));
        }
      }
    }
  ...
  const branchColorMap = useMemo(() => {
    ...
    allBranches.forEach((b, i) => { m[b.id] = PALETTE[i % PALETTE.length]; });
    return m;
  }, [allBranches]);

  const branchNameMap = useMemo(() => {
    const m: Record<string, string> = {};
    allBranches.forEach((b) => { m[b.id] = b.name; });
    return m;
  }, [allBranches]);
  ...
  const indMap = useMemo(() => {
    if (metricMode === "total") return new Map<string, Map<string, number | null>>();
  ...
  const getValue = (reportId: string, branchId: string): number | null => {
    if (metricMode === "total") return aggMap.get(reportId)?.get(branchId)?.total ?? null;
  ...
  const tableBranches = useMemo(() => {
    const active = selectedBranches.size > 0
      ? allBranches.filter((b) => selectedBranches.has(b.id))
      : allBranches;
  ...
  }, [allBranches, selectedReports, aggMap, indMap, metricMode, selectedBranches]);

  // ── Chart data: X = reports, series = branches ──
  const chartData = useMemo(() => {
    const activeBranchIds = selectedBranches.size > 0
  ...
  }, [selectedReports, aggMap, indMap, allBranches, metricMode, selectedBranches]);

  const activeBranchIds = useMemo(
    () => selectedBranches.size > 0 ? [...selectedBranches] : allBranches.map((b) => b.id),
  ...
  const yDomain = useMemo<[number, number]>(() => {
    ...
    const vals = chartData.flatMap((d) =>
      activeBranchIds.map((id) => (typeof d[id] === "number" ? (d[id] as number) : null))
        .filter((v): v is number => v != null),
    );
    if (!vals.length) return [0, 100];
    return [Math.max(0, Math.floor(Math.min(...vals) * 0.9)), Math.ceil(Math.max(...vals) * 1.1)];
  ...
  const toggleAll = () => {
    setSelectedIds(selectedIds.size === historyList.length ? new Set() : new Set(historyList.map((h) => h.id)));
  };
  ...
  const readyForTable = hasData && selectedReports.every((h) => loadedMap.has(h.id));
      ...
      <div className="w-60 shrink-0 border-r-2 border-border flex flex-col bg-muted/20">
        ...
        <div className="flex-1 overflow-y-auto py-1">
          {sortedHistory.map((h) => {
            ...
            return (
              <button key={h.id} onClick={() => toggleId(h.id)}
                ...
                <div className={`mt-0.5 w-4 h-4 shrink-0 rounded border-2 flex items-center justify-center transition-all ${checked ? "bg-emerald-500 border-emerald-500" : "border-border"}`}>
                  {checked && <span className="w-2 h-2 bg-white rounded-sm block" />}
                </div>
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold text-foreground leading-tight truncate">{h.name}</p>
                  <p className="text-[10px] text-muted-foreground mt-0.5">{h.pDate}</p>
                </div>
              </button>
            );
          })}
        ...
        <div className="px-4 py-2 border-t border-border shrink-0">
          <p className="text-[10px] text-muted-foreground">{selectedIds.size} тайлан сонгогдсон</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b-2 border-border bg-card shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-muted/60 text-muted-foreground ml-1">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        {readyForTable && (
          <div className="px-6 py-2.5 border-b border-border bg-muted/20 shrink-0 flex flex-wrap gap-3 items-end">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex h-8 rounded-lg border-2 border-border overflow-hidden">
                {([
                  { key: "total" as MetricMode, label: "Нийт оноо", icon: <BarChart2 className="w-3 h-3" /> },
                  { key: "score" as MetricMode, label: "Score", icon: <Hash className="w-3 h-3" /> },
                  { key: "raw" as MetricMode, label: "Өгөгдөл", icon: <Database className="w-3 h-3" /> },
                ]).map(({ key, label, icon }, idx) => (
                  <button key={key} onClick={() => setMetricMode(key)}
            ...
            {(metricMode === "score" || metricMode === "raw") && (
              <div className="flex flex-col gap-1">
                <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                  {metricMode === "score" ? "Үзүүлэлт (оноо 1–5)" : "Үзүүлэлт (үнэн утга)"}
                </span>
        ...
        {readyForTable && allBranches.length > 0 && (
          <div className="px-6 py-2 border-b border-border bg-muted/10 shrink-0 flex flex-wrap gap-1.5 items-center">
            <span className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground mr-1">Салбар:</span>
            <button onClick={() => setSelectedBranches(new Set())}
              className={`px-2.5 py-1 rounded-full text-[10px] font-bold border-2 transition-colors ${selectedBranches.size === 0 ? "bg-violet-500 text-white border-violet-500" : "border-border text-muted-foreground hover:bg-muted/50"}`}>
              Бүгд
            </button>
            {allBranches.map((b, i) => {
              const active = selectedBranches.has(b.id);
              const color = PALETTE[i % PALETTE.length];
              return (
                <button key={b.id} onClick={() => toggleBranch(b.id)}
                  className={`px-2.5 py-1 rounded-full text-[10px] font-semibold border-2 transition-all ${active ? "text-white" : "border-border text-muted-foreground hover:bg-muted/50"}`}
        ...
        <div className="flex-1 overflow-auto px-6 py-5">
          {selectedIds.size === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground">
              ...
              <p className="text-sm font-medium">Зүүн талаас харьцуулах тайлануудаа сонгоно уу</p>
              <p className="text-xs opacity-60">Хэдэн ч тайлан сонгон харьцуулж болно</p>
            </div>
          ) : fetching ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground">
      ...
      <table className="w-full text-xs border-collapse">
        <thead>
          <tr className="bg-muted/40 border-b-2 border-border">
            ...
            {selectedReports.map((h) => (
              <th key={h.id} className="px-3 py-2.5 text-center font-bold text-[11px] min-w-[110px] border-r border-border last:border-r-0">
                <p className="text-foreground truncate max-w-[160px] mx-auto" title={h.name}>{h.name}</p>
                <p className="text-[10px] font-normal text-muted-foreground mt-0.5">{h.pDate}</p>
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          {tableBranches.map((b, bi) => (
            <tr key={b.id} className={`border-b border-border/50 ${bi % 2 === 1 ? "bg-muted/10" : ""} hover:bg-muted/20 transition-colors`}>
              ...
              {selectedReports.map((h) => {
                ...
                const a = metricMode === "total" ? aggMap.get(h.id)?.get(b.id) ?? null : null;
                return (
                  <td key={h.id} className={`px-3 py-2 text-center border-r border-border last:border-r-0 ${isRaw ? "" : scoreBg(v)}`}>
                    <span className={`text-sm font-black tabular-nums ${isRaw ? "text-foreground" : scoreClass(v)}`}>
                      {isRaw && v != null ? v.toFixed(2) : fmt(v)}
                    </span>
                    {showSub && metricMode === "total" && a && (
                      <div className="flex justify-center gap-2 mt-1">
                        {([["S1", a.s1], ["S2", a.s2], ["S3", a.s3]] as [string, number | null][]).map(([lbl, sv]) => (
                          <div key={lbl} className="text-center">
                            <p className="text-[8px] text-muted-foreground leading-none">{lbl}</p>
                            <p className={`text-[10px] font-bold tabular-nums leading-tight ${scoreClass(sv)}`}>{fmt(sv)}</p>
                          </div>
                        ))}
                      </div>
                    )}
...
function GraphView({
  ...
  if (selectedReports.length < 2) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-muted-foreground gap-2">
        ...
        <p className="text-sm">График харахад хамгийн багадаа 2 тайлан сонгох шаардлагатай</p>
      </div>
    );
  }
  ...
  const CustomDot = (props: any) => {
    ...
    return <circle cx={cx} cy={cy} r={4} fill={stroke} stroke="hsl(var(--background))" strokeWidth={2} />;
  };

  const TooltipContent = ({ active, payload, label }: any) => {
    if (!active || !payload?.length) return null;
    return (
      <div className="rounded-xl border border-border bg-card shadow-xl px-4 py-3 min-w-[200px]">
        ...
        {payload
          ...
          .map((p: any) => (
            <div key={p.dataKey} className="flex items-center gap-2 mb-1">
              ...
              <span className="ml-auto text-[11px] font-bold tabular-nums" style={{ color: p.color }}>
                {isRaw ? Number(p.value).toFixed(2) : Number(p.value).toFixed(2)}
              </span>
            </div>
          ))}
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <ResponsiveContainer width="100%" height={450}>
        {chartType === "line" ? (
          <LineChart data={chartData} margin={{ top: 8, right: 20, left: 10, bottom: 80 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(128,128,128,0.12)" vertical={false} />
            <XAxis dataKey="label" tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }}
              angle={-35} textAnchor="end" height={80} axisLine={{ stroke: "hsl(var(--border))" }} tickLine={false} />
            <YAxis domain={yDomain} ticks={showRefLines ? [0, 1, 2, 3, 4, 5] : undefined}
              tickFormatter={isRaw ? (v) => `${v}` : undefined}
              tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }}
              width={isRaw ? 50 : 32} axisLine={false} tickLine={false} />
            <Tooltip content={<TooltipContent />} />
            <Legend formatter={(val) => <span style={{ fontSize: 11, color: "hsl(var(--muted-foreground))" }}>{branchNameMap[val] ?? val}</span>} wrapperStyle={{ paddingTop: 14 }} />
            {showRefLines && <>
              <ReferenceLine y={3.5} stroke="#ef4444" strokeDasharray="5 3" strokeWidth={1.5}
                label={{ value: "Өндөр ≥3.5", position: "insideTopRight", fontSize: 9, fill: "#ef4444" }} />
              <ReferenceLine y={2.5} stroke="#f59e0b" strokeDasharray="5 3" strokeWidth={1.5}
                label={{ value: "Дунд ≥2.5", position: "insideTopRight", fontSize: 9, fill: "#f59e0b" }} />
            </>}
            {activeBranchIds.map((bId) => (
              <Line key={bId} type="monotone" dataKey={bId}
                stroke={branchColorMap[bId] ?? "#888"} strokeWidth={2.5}
                dot={<CustomDot />} activeDot={{ r: 6, strokeWidth: 2, stroke: "hsl(var(--background))" }}
                connectNulls={false} isAnimationActive animationDuration={600} />
            ))}
          </LineChart>
        ) : (
          <BarChart data={chartData} margin={{ top: 8, right: 20, left: 10, bottom: 80 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(128,128,128,0.12)" vertical={false} />
            <XAxis dataKey="label" tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }}
              angle={-35} textAnchor="end" height={80} axisLine={{ stroke: "hsl(var(--border))" }} tickLine={false} />
            <YAxis domain={yDomain} ticks={showRefLines ? [0, 1, 2, 3, 4, 5] : undefined}
            ...
            {activeBranchIds.map((bId) => (
              <Bar key={bId} dataKey={bId} fill={branchColorMap[bId] ?? "#888"}
                radius={[4, 4, 0, 0]} maxBarSize={32} isAnimationActive animationDuration={600} />
            ))}
          </BarChart>
        )}
      </ResponsiveContainer>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 mt-2">
        {activeBranchIds.map((bId) => {
          const vals = chartData.map((d) => d[bId]).filter((v): v is number => typeof v === "number");
          ...
          return (
            <div key={bId} className="rounded-xl border-2 border-border bg-card px-3.5 py-3"
              style={{ borderLeftColor: color, borderLeftWidth: 4 }}>
              <p className="text-[10px] font-bold truncate mb-2" style={{ color }}>
                {branchNameMap[bId] ?? bId}
              </p>
              <div className="grid grid-cols-3 gap-1 text-center">
                {([["Дундаж", avg], ["Min", min], ["Max", max]] as [string, number][]).map(([lbl, v]) => (
                  <div key={lbl}>
                    <p className="text-[9px] text-muted-foreground mb-0.5">{lbl}</p>
                    <p className={`text-xs font-black tabular-nums ${isRaw ? "text-foreground" : scoreClass(v)}`}>{v.toFixed(2)}</p>
                  </div>
                ))}
              </div>
            </div>
