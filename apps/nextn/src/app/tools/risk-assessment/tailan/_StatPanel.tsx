...
const CustomTooltip = ({
  active, payload, label, branchMap, isData,
}: {
  active?: boolean; payload?: any[]; label?: string;
  branchMap: Record<string, string>; isData: boolean;
}) => {
  if (!active || !payload?.length) return null;
  return (
    <div className="rounded-xl border border-border bg-card shadow-xl px-4 py-3 min-w-[200px] max-w-xs">
        ...
        .map((p) => (
          <div key={p.dataKey} className="flex items-center gap-2 mb-1">
            ...
            <span className="ml-auto text-[11px] font-bold tabular-nums" style={{ color: p.color }}>
              {isData ? `${Number(p.value).toFixed(2)}%` : Number(p.value).toFixed(2)}
            </span>
          </div>
        ))}
    </div>
...
export default function StatPanel({
  ...
  const [loadedMap, setLoadedMap] = useState<Map<string, LoadedEntry>>(new Map());

  // ── Realtime mode state ──
  const rtCacheRef = useRef<Map<string, LoadedEntry>>(new Map());
  const [rtDates, setRtDates] = useState<string[]>([]);
  const [rtLoadedMap, setRtLoadedMap] = useState<Map<string, LoadedEntry>>(new Map());
  ...
  useEffect(() => { setToIdx(Math.max(0, sortedHistory.length - 1)); }, [sortedHistory.length]);

  // ── Fetch: history mode ──
  const doFetchHistory = useCallback((entries: RiskHistoryEntry[]) => {
    if (!entries.length) { setLoadedMap(new Map(cacheRef.current)); return; }
    setFetching(true);
    Promise.all(
      entries.map((h) =>
        riskApi.getHistory(h.id)
          .then((res) => { cacheRef.current.set(h.id, { rows: res.rows ?? [] }); })
          .catch(() => { cacheRef.current.set(h.id, { rows: [] }); }),
      ),
  ...
  // ── Fetch: realtime mode — dates list ──
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!toFetch.length) { setRtLoadedMap(new Map(rtCacheRef.current)); return; }
    setFetching(true);
    Promise.all(
      toFetch.map((d) =>
        riskApi.getRealtime(d)
          .then((res) => { rtCacheRef.current.set(d, { rows: res.rows ?? [] }); })
          .catch(() => { rtCacheRef.current.set(d, { rows: [] }); }),
      ),
    ).finally(() => { setRtLoadedMap(new Map(rtCacheRef.current)); setFetching(false); });
  ...
  }, [useRealtime, activeRtDates, rtLoadedMap, sortedHistory, fromIdx, toIdx, loadedMap]);
    ...
    for (const e of rangeEntries) {
      for (const r of e.rows) {
        if (r.BRANCHID && !map.has(r.BRANCHID))
          map.set(r.BRANCHID, String(r.BRANCHNAME ?? r.BRANCHID));
      }
    }
    ...
    return rangeEntries.map((e) => {
      ...
      if (mode === "total") {
        ...
        for (const r of e.rows) {
          ...
          if (!isNaN(raw)) point[r.BRANCHID] = raw;
        }
      }
      return point;
    });
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-background text-foreground">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b-2 border-border shrink-0 bg-card">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-base font-bold">Статистик · Өөрчлөлтийн чиг хандлага</h2>
          {useRealtime && (
        ...
        <div className="flex items-center gap-2">
          <button onClick={forceRefetch} title="Дахин ачааллах" disabled={fetching}
      ...
      <div className="px-6 py-3 border-b border-border bg-muted/20 shrink-0 flex flex-wrap gap-3 items-end">
        ...
        {useRealtime ? (
          <>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Эхлэх</span>
              <select value={rtFrom} onChange={(e) => setRtFrom(e.target.value)}
                ...
                {rtDates.map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Дуусах</span>
              <select value={rtTo} onChange={(e) => setRtTo(e.target.value)}
                className="h-8 px-2.5 text-xs rounded-lg border border-border bg-background focus:outline-none focus:ring-2 focus:ring-emerald-500/30 min-w-[140px]">
                {rtDates.map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
          ...
          <>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Эхлэх огноо</span>
              <select value={fromIdx}
                ...
                {sortedHistory.map((h, i) => <option key={h.id} value={i}>{h.pDate} — {h.name}</option>)}
              </select>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Дуусах огноо</span>
              <select value={toIdx}
        ...
        {/* Mode */}
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Харах утга</span>
          <div className="flex h-8 rounded-lg border-2 border-border overflow-hidden">
            {([
              { key: "total" as Mode, label: "Нийт оноо", icon: <BarChart2 className="w-3 h-3" /> },
              { key: "score" as Mode, label: "Score", icon: <Hash className="w-3 h-3" /> },
              { key: "data" as Mode, label: "Өгөгдөл", icon: <Database className="w-3 h-3" /> },
              ...
              <button key={key} onClick={() => setMode(key)}
                className={`px-3 text-xs font-semibold transition-colors flex items-center gap-1.5 ${idx > 0 ? "border-l border-border" : ""} ${
                  mode === key ? "bg-emerald-500 text-white" : "text-muted-foreground hover:bg-muted/50"
                }`}>
                {icon}{label}
              </button>
            ))}
        ...
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Диаграм</span>
          <div className="flex h-8 rounded-lg border-2 border-border overflow-hidden">
        ...
        <div className="px-6 py-2 border-b border-border bg-muted/10 shrink-0 flex flex-wrap gap-1.5 items-center">
          ...
          <button onClick={() => setSelectedBranches(new Set())}
            className={`px-2.5 py-1 rounded-full text-[10px] font-bold border-2 transition-colors ${
              selectedBranches.size === 0 ? "bg-emerald-500 text-white border-emerald-500" : "border-border text-muted-foreground hover:bg-muted/50"
            }`}>
            Бүгд
          </button>
          {allBranches.map((b, i) => {
            ...
            return (
              <button key={b.id}
                onClick={() => setSelectedBranches((prev) => { const n = new Set(prev); if (n.has(b.id)) n.delete(b.id); else n.add(b.id); return n; })}
          ...
          <>
            ...
            <ResponsiveContainer width="100%" height={420}>
              {chartType === "line" ? (
                <LineChart data={chartData} margin={{ top: 8, right: 20, left: 10, bottom: 80 }}>
                  ...
                  <YAxis domain={yDomain} ticks={isDataMode ? undefined : [0, 1, 2, 3, 4, 5]}
                    tickFormatter={isDataMode ? (v) => `${v}%` : undefined}
                    tick={{ fontSize: 11, fill: "hsl(var(--muted-foreground))" }}
                    width={isDataMode ? 55 : 32} axisLine={false} tickLine={false} />
                  <Tooltip content={<CustomTooltip branchMap={branchNameMap} isData={isDataMode} />} />
                ...
                <BarChart data={chartData} margin={{ top: 8, right: 20, left: 10, bottom: 80 }}>
                  ...
                  {activeBranchIds.map((bId) => (
                    <Bar key={bId} dataKey={bId} fill={branchColorMap[bId] ?? "#888"}
                      radius={[5, 5, 0, 0]} maxBarSize={36} isAnimationActive animationDuration={600} />
                  ))}
                </BarChart>
              )}
            </ResponsiveContainer>
            ...
            <div className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
              {activeBranchIds.map((bId) => {
                ...
                const fmt = (n: number) => isDataMode ? `${n.toFixed(1)}%` : n.toFixed(2);
                return (
                  <div key={bId} className="rounded-xl border-2 border-border bg-card px-3.5 py-3"
                    style={{ borderLeftColor: color, borderLeftWidth: 4 }}>
                    <p className="text-[10px] font-bold truncate mb-2" style={{ color }}>{branchNameMap[bId] ?? bId}</p>
                    <div className="grid grid-cols-3 gap-1 text-center">
                      <div>
                        <p className="text-[9px] text-muted-foreground mb-0.5">Дундаж</p>
                        <p className="text-xs font-black tabular-nums text-foreground">{fmt(avg)}</p>
                      </div>
                      <div>
                        <p className="text-[9px] text-muted-foreground mb-0.5">Сүүлд</p>
                        <p className="text-xs font-black tabular-nums text-foreground">{fmt(last)}</p>
                      </div>
                      <div>
                        <p className="text-[9px] text-muted-foreground mb-0.5">Өөрчлөлт</p>
                        <p className={`text-xs font-black tabular-nums ${
                          isDataMode
                            ? delta > 0.5 ? "text-emerald-500" : delta < -0.5 ? "text-red-500" : "text-muted-foreground"
                            : delta > 0.05 ? "text-red-500" : delta < -0.05 ? "text-emerald-500" : "text-muted-foreground"
                        }`}>
                          {delta > 0 ? "+" : ""}{fmt(delta)}
                        </p>
                      </div>
