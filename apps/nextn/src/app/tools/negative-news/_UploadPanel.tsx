...
import { getApiErrorMessage, negativeNewsApi } from "@/lib/api";
...
export function UploadPanel({ onImported }: { onImported: () => void }) {
  ...
  const readFile = async (file: File) => {
    ...
    setProgress(null);
    try {
      const parsed = await parseNewsWorkbook(await file.arrayBuffer());
  ...
  const runImport = async () => {
    ...
    try {
      for (const [i, rows] of chunks.entries()) {
        const res = await negativeNewsApi.importRows({
          batchId,
          fileName,
                ...
                <tbody>
                  {sheet.rows.slice(0, 8).map((r, i) => (
                    <tr key={i}>
                      ...
                      <td className={cn(toolTdClass, "max-w-[520px]")}>
