...
/** Оруулсан багцуудын түүх — алдаатай файлыг (дүгнэлтийн хамт) буцааж болно. */
export function BatchHistory({
  batches,
  loading,
  onChanged,
}: {
  batches: DbChangeBatch[] | null;
  loading: boolean;
  onChanged: () => void;
}) {
  const { t } = useLanguage();
  const { toast } = useToast();
  const [pending, setPending] = useState<DbChangeBatch | null>(null);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await dbChangesApi.deleteBatch(pending.batchId);
                ...
                <tr key={b.batchId}>
                  ...
                  >
                    {b.newRows + b.duplicateRows > 0
                      ? `${b.minTime.slice(0, 10)} — ${b.maxTime.slice(0, 10)}`
                      : "—"}
                  </td>
                  <td
                    className={cn(
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pending?.fileName} — {t("dbcDeleteConfirm")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
