  ...
  return (
    ...
    >
        ...
        <div className="relative" ref={searchBoxRef}>
          <div className="relative">
            <input
              ...
              className="w-full h-11 bg-muted/60 border border-border rounded-xl pl-4 pr-11 text-sm text-white placeholder:text-muted-foreground/70 focus:outline-none focus:border-amber-500/60 focus:ring-1 focus:ring-amber-500/20 transition-all"
            />
          ...
          <AnimatePresence>
            {showDropdown && searchResults.length > 0 && (
              <motion.div
                ...
                className="absolute z-50 w-full mt-2 bg-card/95 backdrop-blur-xl border border-amber-500/20 rounded-2xl shadow-2xl overflow-hidden"
              >
        ...
        {incoming.length === 0 && outgoing.length === 0 ? (
          ...
          <div className="space-y-2">
            ...
            {outgoing.map((inv) => (
              <div
                key={inv.id}
                className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-card/50 border border-border/30"
              >
                <div>
                  <p className="text-sm text-foreground/80">{inv.toUserName}</p>
                ...
                <button
                  onClick={() => declineInvite(inv.id)}
                  disabled={decliningId === inv.id}
                  className="p-1.5 rounded-lg bg-muted/60 hover:bg-muted/60 text-muted-foreground transition-all shrink-0"
                  title="Цуцлах"
                >
                  <X className="w-3.5 h-3.5" />
                </button>
      ...
      {activeGames.filter((g) => g.status === "active").length > 0 && (
        <div className="md:col-span-2 bg-muted/50 border border-border/50 rounded-2xl p-5 space-y-3">
          <h2 className="text-sm font-semibold text-foreground/80 flex items-center gap-2">
      ...
      {/* My stats card */}
      {myStats && (
        <div className="md:col-span-2 bg-muted/50 border border-border/50 rounded-2xl p-5 space-y-4">
          <h2 className="text-sm font-semibold text-foreground/80 flex items-center gap-2">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl py-3">
              ...
              <p className="text-[10px] text-muted-foreground/70 mt-0.5">Ялалт</p>
            </div>
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl py-3">
              <p className="text-xl font-bold text-yellow-400">
                {myStats.draws}
              </p>
              <p className="text-[10px] text-muted-foreground/70 mt-0.5">Тэнцсэн</p>
            </div>
            <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl py-3">
              <p className="text-xl font-bold text-rose-400">
                {myStats.losses}
              </p>
              <p className="text-[10px] text-muted-foreground/70 mt-0.5">Хожигдсон</p>
            </div>
          </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
                {myStats.games.slice(0, 8).map((g) => (
                  <div
                    key={g.id}
                    className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-card/50"
                  >
                    <span
      ...
      {loadingStats && !myStats && (
        <div className="md:col-span-2 bg-muted/50 border border-border/50 rounded-2xl p-5 flex items-center justify-center h-32">
