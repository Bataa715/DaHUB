...
export function WordPreview({
  ...
  return (
    ...
    >
          ...
          {plannedTasks
            ...
            .map((t, idx) => (
              <div key={t._id} style={{ marginBottom: "6pt" }}>
                <span style={{ fontWeight: "bold" }}>{idx + 1}. {t.title}</span>
                {t.description?.trim() && (
                  <div
                    style={{
                      marginLeft: "16pt",
                ...
                {t.images?.length > 0 && (
                  <div style={{ marginTop: "6pt" }}>
                    {t.images.map((img) => (
                      <div key={img.id} style={{ textAlign: "center", marginBottom: "8pt", pageBreakInside: "avoid" }}>
                        <img src={img.dataUrl} alt="" style={{ width: `${img.width}%`, maxWidth: "100%", display: "inline-block" }} />
                        <div style={{ fontSize: "9pt", fontStyle: "italic", marginTop: "3pt" }}>
                          Зураг {imgCounter.n++}.
                        </div>
                      </div>
                    ))}
            ...
            <>
              {section1Dashboards.map((t, idx) => (
                <tr key={t._id}>
                  ...
                  <td style={tdStyle("30%")}>{t.summary ? parseContent(t.summary, tableCounter) : ""}</td>
                </tr>
              ))}
              <tr>
                <td
      ...
      {section1Dashboards.some((t) => t.images?.length > 0) && (
        <div style={{ marginBottom: "8pt" }}>
          {section1Dashboards
            ...
            .map((t) =>
              t.images.map((img) => (
                <div key={img.id} style={{ textAlign: "center", marginBottom: "10pt", pageBreakInside: "avoid" }}>
                  <img src={img.dataUrl} alt="" style={{ width: `${img.width}%`, maxWidth: "100%", display: "inline-block" }} />
                  <div style={{ fontSize: "9pt", fontStyle: "italic", marginTop: "3pt" }}>
                    Зураг {imgCounter.n++}.
                  </div>
      ...
      <div style={{ fontSize: "9pt", fontStyle: "italic", textAlign: "center", marginBottom: "2pt" }}>
        Хүснэгт {tableCounter.n++}.
      </div>
        ...
        <tbody>
          {section3AutoTasks.length === 0 ? (
            ...
            <>
              ...
              <tr>
                <td colSpan={3} style={{ ...tdStyle("80%"), fontWeight: "bold", textAlign: "center" }}>
                  Дундаж үнэлгээ
                </td>
                <td style={{ ...tdStyle("20%"), fontWeight: "bold", textAlign: "center" }}>
                  {(() => {
          ...
          {section4Trainings.length === 0 ? (
            ...
            section4Trainings.map((t, idx) => (
              <tr key={t._id}>
                ...
                <td style={{ ...tdStyle("15%"), textAlign: "center" }}>{t.organizer}</td>
          ...
          {section5Tasks.length === 0 ? (
            ...
            section5Tasks.map((t, idx) => (
              <tr key={t._id}>
                ...
                <td style={tdStyle("60%")}>{t.completedWork ? parseContent(t.completedWork, tableCounter) : ""}</td>
              </tr>
            ))
          )}
        </tbody>
