...
import type { Column, DataDocDatabase, DatabaseTable, DatabaseSchema } from "./data-doc-types";
...
export function parseSchema(): DatabaseSchema {
  ...
  const describedColumns = databases.reduce(
    (s, db) => s + db.tables.reduce((t, tbl) => t + tbl.columns.filter((c) => c.description.length > 0).length, 0),
    0,
  );
