...
import {
  Bell, AlertTriangle, X, Search, Flag,
  BellDot, ChevronLeft,
} from 'lucide-react';
...
export default function ABSidebar() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      ...
      if (
        ...
        notifPopupRef.current && !notifPopupRef.current.contains(t)
      ) setNotifOpen(false);
    };
  ...
  useEffect(() => {
    const load = () => abFetchNotifications(20).then(setNotifications).catch(() => {});
    load();
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-[220px] bg-surface-card border-r border-surface-border flex flex-col z-40">
      ...
      <div className="px-4 py-4 border-b border-surface-border">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[9px] text-txt-dim uppercase tracking-wide">Голомт Банк</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-2.5 py-2 space-y-0.5 overflow-y-auto">
        <p className="text-[9px] font-semibold text-txt-dim uppercase tracking-wider px-2 pt-1 pb-1.5">Хэрэгслүүд</p>
      ...
      {/* Notification popup */}
      {notifOpen && (
        <div ref={notifPopupRef} className="fixed left-[230px] bottom-16 w-[320px] max-h-[380px] bg-surface-card border border-surface-border rounded-xl shadow-2xl overflow-hidden z-[60]">
          <div className="flex items-center justify-between px-4 py-3 border-b border-surface-border">
            <span className="text-xs font-bold text-txt">Мэдэгдэл</span>
            <button onClick={() => setNotifOpen(false)} className="text-txt-dim hover:text-txt"><X size={13} /></button>
          </div>
          {notifications?.criticalCount > 0 && (
            <div className="mx-3 mt-2 px-3 py-2 bg-red-500/10 border border-red-500/20 rounded-lg flex items-center gap-2">
              ...
              <span className="text-[11px] text-red-300 font-medium">{notifications.criticalCount} өндөр эрсдэл</span>
            </div>
          )}
          <div className="overflow-y-auto max-h-[280px] py-1">
            {!notifications || notifCount === 0 ? (
              <div className="text-center py-8">
                <Bell size={18} className="text-txt-dim mx-auto mb-2 opacity-40" />
                <p className="text-xs text-txt-dim">Мэдэгдэл байхгүй</p>
              </div>
            ) : (
              notifications.items?.map((n: any) => (
                <div key={n.id} className="flex items-start gap-3 px-4 py-2.5 border-b border-surface-border/50 last:border-0">
